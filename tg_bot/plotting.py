...
def plot_water_history(
    ...
    bars = plt.bar(dates, water_consumed, label='Consumed', color='#3498db', alpha=0.8, width=0.6)
    ...
    plt.ylabel('Water (ml)', fontsize=12, fontweight='bold')
    plt.title('Water Intake History', fontsize=14, fontweight='bold')
...
def plot_calorie_history(
    ...
    bars1 = plt.bar(x - width/2, calories_consumed, width, label='Consumed', color='#e74c3c', alpha=0.8)
    bars2 = plt.bar(x + width/2, calories_burned, width, label='Burned', color='#f39c12', alpha=0.8)
    ...
    plt.xlabel('Date', fontsize=12, fontweight='bold')
    plt.ylabel('Calories (kcal)', fontsize=12, fontweight='bold')
    plt.title('Calorie History', fontsize=14, fontweight='bold')
    plt.legend(fontsize=10, loc='upper left')
    plt.grid(True, alpha=0.3, axis='y')
